...
class OI_FLUX:
    ...
    def __init__(self, timeobs, timemjd, int_time, ins, fluxdata, fluxerr, flag, target, array=None, station=None):
    ...
    def __eq__(self, other):
        ...
        return not (
                    ...
                    (not _array_eq(self.fluxdata, other.fluxdata)) or
                    (not _array_eq(self.fluxerr, other.fluxerr)) or
...
class OI_VIS:
    ...
    def __init__(self, timeobs, timemjd, int_time, ins, arrname, visamp, visamperr, visphi, visphierr, flag, ucoord, vcoord, wavelength, target, array=None, station=(None,None), cflux=None, cfluxerr=None):
        self.timeobs = timeobs
        self.timemjd = timemjd
        self.ins = ins
...
class OI_VIS2:
    ...
    """
    def __init__(self, timeobs, timemjd, int_time, ins, vis2data, vis2err, flag, ucoord, vcoord, wavelength,
...
class OI_T3:
    ...
    def __init__(self, timeobs, timemjd, int_time, ins, t3amp, t3amperr, t3phi, t3phierr, flag, u1coord,
...
class OI_STATION:
    ...
    def __init__(self, tel_name=None, sta_name=None, sta_index=None, diameter=None, staxyz=[None, None, None]):
        ...
        self.staxyz = staxyz
    #self.mntsta = mntsta
# for different merged files, some still contain mntsta, some delete it (as Frank did). So here needs to be changed accordingly.
...
class OI_ARRAY:
    ...
    def __init__(self, frame, arrxyz, stations=()):
        self.frame = frame
        self.arrxyz = arrxyz
        print("FG: in oifits_new.py arrxyz=",arrxyz)
        #self.station = stations
        self.station = np.empty(0)
        for station in stations:
            try:
                tel_name, sta_name, sta_index, diameter, staxyz, mntsta = station
            except ValueError:
                tel_name, sta_name, sta_index, diameter, staxyz = station
            self.station = np.append(self.station, OI_STATION(tel_name=tel_name, sta_name=sta_name, sta_index=sta_index, diameter=diameter, staxyz=staxyz))
...
class oifits:
    ...
    def save(self, filename):
        ...
        for insname, wavelength in self.wavelength.items():
            ...
            hdulist.append(hdu)
        print("FG: wavelengthmap=",wavelengthmap)
        ...
        for arrname, array in self.array.items():
            ...
            if array.station.size:
                for i, station in enumerate(array.station, 1):
                    print("FG: oifits_new.py id(station)=",id(station),"station[0]=",station)
                    print("FG: array.station=",array.station,array.station.shape,array.station[0].sta_name)
                    stationmap[id(station)] = i
                    print("FG: stationmap=",stationmap)
        ...
        if self.vis.size:
            ...
            for vis in self.vis:
                ...
                if vis.timeobs:
                    print("FG: in oifits_new.py vis.timeobs=",vis.timeobs,"refdate=",refdate)
                    #FG:
                ...
                if vis.station[0] and vis.station[1]:
                    print("FG: oifits_new.py vis.station[0]=",vis.station[0],vis.station[1])
                    print("FG: oifits_new.py stationmap=",stationmap)
                    print("FG: id(vis.station[0])=",id(vis.station[0]))
                    #data['sta_index'].append([stationmap[id(vis.station[0])], stationmap[id(vis.station[1])]])
                    # FG
                    data['sta_index'].append([vis.station[0],vis.station[1]])
                else:
        ...
        if self.vis2.size:
            ...
            for vis in self.vis2:
                nwave = vis.wavelength.eff_wave.size
                if vis.array:
                    print("FG: vis.wavelength=",vis.wavelength)
                    key = (arraymap[id(vis.array)], wavelengthmap[id(vis.wavelength)])
                else:
                    key = (None, wavelengthmap[id(vis.wavelength)])
                if key in list(tables.keys()):
...
def open(filename, quiet=False):
    """Open an OIFITS file."""
    #adding OI_FLUX data by 20191008
    ...
    for hdu in hdulist:
        ...
        if hdu.name == 'OI_VIS':
            print("comments in oifits_new.py")
            for row in data:
                ...
                if k == 6:
                    ...
                    #print "row.field('TIME')=", row.field('TIME'), k, j, header['INSNAME']
                    #print np.around(row.field('TIME'), 2)
                    #print datetime.timedelta(seconds=np.around(row.field('TIME'), 2))
                    print("row.field('MJD')=", row.field('MJD'), k, j, header['INSNAME'])
                    j = j + 0.5
                k = k + 1
                int_time = row.field('INT_TIME')
                timemjd = row.field('MJD')
                ins = header['INSNAME']
                ...
                if array:
                    ...
                    station = [None, None]
                newobj.vis = np.append(newobj.vis, OI_VIS(timeobs=timeobs, timemjd=timemjd, int_time=int_time, visamp=visamp, ins=ins, arrname=arrname,
            ...
            for row in data:
                ...
                if array:
                    ...
                    station = [None, None]
                newobj.vis2 = np.append(newobj.vis2, OI_VIS2(timeobs=timeobs, timemjd=timemjd, int_time=int_time, vis2data=vis2data, ins=ins,
            ...
            for row in data:
                ...
                newobj.t3 = np.append(newobj.t3, OI_T3(timeobs=timeobs, timemjd=timemjd, int_time=int_time, t3amp=t3amp, ins=ins,
            ...
            for row in data:
                ...
                ins = header['INSNAME']
                try:
                    fluxdata = np.reshape(row.field('FLUXDATA'), -1)
                except KeyError:
                    fluxdata = np.reshape(row.field('FLUX'), -1)
                fluxerr = np.reshape(row.field('FLUXERR'), -1)
                flag = np.reshape(row.field('FLAG'), -1)
                target = targetmap[row.field('TARGET_ID')]
                if array:
                    sta_index = row.field('STA_INDEX')
                    #print "sta_index=",sta_index
                    s1 = array.station[sta_indices[arrname] == sta_index][0]
                    #print "s1=",s1
                    #s2 = array.station[sta_indices[arrname] == sta_index[1]][0]
                    station = s1
                else:
                    station = None
                newobj.flux = np.append(newobj.flux, OI_FLUX(timeobs=timeobs, timemjd=timemjd, int_time=int_time, fluxdata=fluxdata, ins=ins, fluxerr=fluxerr, flag=flag, target=target, array=array, station=station))
