...
#        print 'final data shape: ',self.data.shape
    ...
    def save_uvfits(self, fname):
        ...
        # Load the array data
        tarr = self.tarr
        print("20180314: tarr.shape=",tarr.shape)
        print("tarr=")
        print(tarr)
        tnames = tarr['site']
        ...
        #here this 2 corresponds to counting rr and ll as two records, which really should be one record for dual polarization mode
        ...
        print("self.timemjd.shape=", self.timemjd.shape)
        jds = np.round(self.timemjd)+2400000.5
        fractimes = self.timemjd - np.fix(self.timemjd)
        print("!!!!!fractimes.shape 1 =",fractimes.shape)
        ...
        #if dualpol == True:
        rr = np.zeros(ndat*nwav,dtype=complex)
        ll = np.zeros(ndat*nwav,dtype=complex)
        weight_rr = np.zeros(ndat*nwav)
        weight_ll = np.zeros(ndat*nwav)
        ...
        kr = 0
        kl = 0
        print("len(self.datatable['vis'])=",len(self.datatable['vis']))
        for i in range(len(self.datatable['vis'])):
            if self.ins[i].find('P1') != -1:
            ...
            if self.ins[i].find('P2') != -1:
                ...
                kl = kl + 1
            if dualpol == False:
                #print "*** dualpol == False, only 1 polarizaiton data, put in rr, leaving ll to be zero ***"
                rr[kr] = rr[kr] + self.datatable['vis'][i]
                weight_rr[kr] = weight_rr[kr] + 1 / (2 * self.datatable['sigma'][i]**2)
                print("kr=",kr,"weight_rr[kr]=",weight_rr[kr])
                kr = kr + 1
        print("kr=",kr)
        ...
        u=np.reshape(u,(ndatt/nwav,nwav))
        v=np.reshape(v,(ndatt/nwav,nwav))
        bl=np.reshape(bl,(ndatt/nwav,nwav))
        jds=np.reshape(jds,(ndatt/nwav,nwav))
        fractimes=np.reshape(fractimes,(ndatt/nwav,nwav))
        print("!!!!!fractimes.shape 2=",fractimes.shape)
        print(fractimes[:,0])
        tints=np.reshape(tints,(ndatt/nwav,nwav))
        el1=np.reshape(el1,(ndatt/nwav,nwav))
        el2=np.reshape(el2,(ndatt/nwav,nwav))
        tau1=np.reshape(tau1,(ndatt/nwav,nwav))
        tau2=np.reshape(tau2,(ndatt/nwav,nwav))
        ...
        print("!!!!!fractimes.shape 3=",fractimes.shape)
        tints = tints[:,0]
        el1 = el1[:,0]
        el2 = el2[:,0]
        tau1 = tau1[:,0]
        ...
        if dualpol == True:
            print("u.shape=",u.shape)
            print("u=",u)
            u = np.reshape(u,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            v = np.reshape(v,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            bl = np.reshape(bl,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            jds = np.reshape(jds,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            fractimes = np.reshape(fractimes,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            print("!!!!!fractimes.shape 4=",fractimes.shape)
            print(fractimes[:,0])
            tints = np.reshape(tints,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            el1 = np.reshape(el1,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            el2 = np.reshape(el2,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            tau1 = np.reshape(tau1,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            tau2 = np.reshape(tau2,(ndatt/nwav/6/n_sub_frame,6*n_sub_frame))
            ...
            print("!!!!!fractimes.shape 5=",fractimes.shape)
            print(fractimes)
    ...
    def save_oifits(self, fname, flux=1.0):
        ...
        print("biarr.shape=",biarr.shape)
        
        print("biarr[0]=",biarr[0])
        ...
        u = data['u']
        v = data['v']
        print("FG: amp.shape=",amp.shape[0]/nwav)
        print("FG: u.shape=",u.shape)
        print("FG: intTime.shape=",intTime.shape)
        ...
        print("now reshaping the visibility data into 2-D")
        
        amp = amp.reshape(amp.shape[0]/nwav,nwav)
        phase = phase.reshape(phase.shape[0]/nwav,nwav)
        viserror = viserror.reshape(viserror.shape[0]/nwav,nwav)
        u = u.reshape(u.shape[0]/nwav,nwav)
        v = v.reshape(v.shape[0]/nwav,nwav)
        
        ant1 = ant1.reshape(ant1.shape[0]/nwav,nwav)
        ant2 = ant2.reshape(ant2.shape[0]/nwav,nwav)
        
        print("FG: amp.shape=",amp.shape)
        

        
        # convert times to datetime objects
        time = data['time'][0]
        print("FG: time.shape=",time.shape)
        print("FG: time=",time)
        ...
        dttime = datetime.datetime.utcfromtimestamp(time*60*60)
        print("FG: dttime=",dttime)
        # FG: here I didn't use the dttime in below
        
        # get the bispectrum information
        bi = biarr['bispec']
        print("FG: bi.shape=",bi.shape)
        t3amp = np.abs(bi);
        ...
        print("FG: antOrder=",antOrder)
        print("FG: ant1=",ant1)
        print("FG: ant1[0]=",ant1[0])
        print("FG: ant2=",ant2)
        print("FG: ant1.shape=",ant1.shape)

        # todo: check that putting the negatives on the phase and t3phi is correct
        print("self.rf=",self.rf)
        print("self.bw=",self.bw)
        print("FG: in vlbi_imaging_utils0.py data['time']=",data['time'])
        #print "FG: in vlbi_imaging_utils0.py dttime=",dttime
        print("FG: in vlbi_imaging_utils0.py u.shape=",u.shape)
        print("FG: in vlbi_imaging_utils0.py amp.shape=",amp.shape)
        print("FG: in vlbi_imaging_utils0.py self.rf.shape=",self.rf.shape)
        print("FG: in vlbi_imaging_utils0.py ### the data being written are: ###")
        print("FG: in vlbi_imaging_utils0.py self.ra=",self.ra)
        print("FG: in vlbi_imaging_utils0.py self.dec=",self.dec)
        print("FG: in vlbi_imaging_utils0.py self.rf=",self.rf)
        print("FG: in vlbi_imaging_utils0.py self.bw=",self.bw)
        print("FG: in vlbi_imaging_utils0.py intTime=",intTime)
        print("FG: in vlbi_imaging_utils0.py amp=",amp)
        print("FG: in vlbi_imaging_utils0.py viserror=",viserror)
        print("FG: in vlbi_imaging_utils0.py phase=",phase)
        print("FG: in vlbi_imaging_utils0.py u=",u)
        print("FG: in vlbi_imaging_utils0.py v=",v)
        print("FG: in vlbi_imaging_utils0.py ant1=",ant1)
        print("FG: in vlbi_imaging_utils0.py ant2=",ant2)
        print("FG: in vlbi_imaging_utils0.py time=",time)
        print("FG: in vlbi_imaging_utils0.py t3amp=",t3amp)
        print("FG: in vlbi_imaging_utils0.py t3amperr=",t3amperr)
        print("FG: in vlbi_imaging_utils0.py t3phi=",t3phi)
        print("FG: in vlbi_imaging_utils0.py t3phi=",t3phierr)
        print("FG: in vlbi_imaging_utils0.py uClosure",uClosure)
        print("FG: in vlbi_imaging_utils0.py vClosure=",vClosure)
        print("FG: in vlbi_imaging_utils0.py antOrder=",antOrder)
        print("FG: in vlbi_imaging_utils0.py dttimeClosure=",dttimeClosure)
        print("FG: in vlbi_imaging_utils0.py antennaNames=",antennaNames)
        print("FG: in vlbi_imaging_utils0.py antennaDiam=",antennaDiam)
        print("FG: in vlbi_imaging_utils0.py antennaX=",antennaX)
        print("FG: in vlbi_imaging_utils0.py antennaY=",antennaY)
        print("FG: in vlbi_imaging_utils0.py antennaZ=",antennaZ)
...
def load_obs_uvfits(filename, flipbl=False):
    ...
    ### FG: in order to properly calculate the central wavelength and effective bandwidth, here I hard-wired these two quantities:

    rf_ori = np.array([149896.22937845,147173.51468036,144547.96203037,142014.43106196,139568.18203551,137204.78094105,134920.10290984,132710.25235481,130571.62532683,128500.83313124,126494.71082406,124550.25203453,122664.66811931,120835.33608481])
    rf = rf_ori*1e9

    bw = np.array([5282483142656.,8989059317760.,9362903924736.,8605443031040.,8161437155328.,8453794299904.,8649341140992.,7831954128896.,7028426670080.,6420782120960.,6184318795776.,5990358450176.,5624247091200.,4675899752448.])

    # Mask to screen bad data
    rrweight = data['DATA'][:,0,0,:,0,0,2]
    llweight = data['DATA'][:,0,0,:,0,1,2]
    print("rrweight.shape=",rrweight.shape)
    ...
    # Sites - add names
    t1 = data['BASELINE'].astype(int)/256
    t2 = data['BASELINE'].astype(int) - t1*256
    t1 = t1 - 1
    t2 = t2 - 1
    ...
    # Get vis data
    rr = data['DATA'][:,0,0,:,0,0,0] + 1j*data['DATA'][:,0,0,:,0,0,1]
    ll = data['DATA'][:,0,0,:,0,1,0] + 1j*data['DATA'][:,0,0,:,0,1,1]
    print("rr.shape=",rr.shape)
    ...
    qsig = 0*ivis#np.sqrt(rlsig**2 + lrsig**2)/2.0
    usig = 0*ivis#qsig
    ...
    print("ivis.shape=",ivis.shape)

    times = np.transpose(np.array([times for i in range(14)]))
    tints = np.transpose(np.array([tints for i in range(14)]))
    t1 = np.transpose(np.array([t1 for i in range(14)]))
    t2 = np.transpose(np.array([t2 for i in range(14)]))
    el1 = np.transpose(np.array([el1 for i in range(14)]))
    el2 = np.transpose(np.array([el2 for i in range(14)]))
    tau1 = np.transpose(np.array([tau1 for i in range(14)]))
    tau2 = np.transpose(np.array([tau2 for i in range(14)]))
    u = np.transpose(np.array([u for i in range(14)]))
    v = np.transpose(np.array([v for i in range(14)]))
    print("u.shape=",u)
    print("v.shape=",v)
    print("times.shape=",times)
    print("tints.shape=",tints)
    print("t1.shape=",t1)
    print("t2.shape=",t2)
    print("el1.shape=",el1)
    print("el2.shape=",el2)
    print("tau1.shape=",tau1)
    print("tau2.shape=",tau2)
    print("ivis.shape=",ivis)
    print("qvis.shape=",qvis)
    print("uvis.shape=",uvis)
    ...
    datatable = np.array([ (times[i], tints[i], t1[i], t2[i], el1[i], el2[i], tau1[i], tau2[i], u[i], v[i], ivis[i], qvis[i], uvis[i], sigma[i]) for i in range(len(times))], dtype=DTPOL)

    #datatable = np.array([ ivis[i] for i in range(len(times))])
    ...
    return Obsdata(ra, dec, rf, bw, datatable, tarr, timemjd=mjd, dateobs=jds, ins=ins, arrname=arrname, nwav=nwav, source=src, mjd=mjd, ampcal=True, phasecal=True)
...
def load_obs_oifits(filename, flux=1.0, specavg=-1):
    ...
    if specavg==1:
        if nwav_ori == 210:
            ...
            print("specavg = 1, re-binning the data with every 3 channels")
            n_bin = 3
            nwav = nwav_ori/n_bin
            print("now nwav=",nwav)
            rf = np.zeros(nwav)
...
#            amp_ori = np.array([vis_data[i].visamp for i in range(len(vis_data))])
#            print "amp_ori[0,:]=",amp_ori[0,:]
#            amp = np.zeros(len(vis_data)*nwav).reshape(len(vis_data),nwav)
#            print "amp_ori.shape=", amp_ori.shape


#            #now for the phase
#            phase_ori = np.array([vis_data[i].visphi for i in range(len(vis_data))])
#            #here I changed the sign of the phase in corresponding to the swap of baseline direction
#            phase_ori = -1*phase_ori
#           print "phase_ori[0,:]=",phase_ori[0,:]
#            phase = np.zeros(len(vis_data)*nwav).reshape(len(vis_data),nwav)

#            for i in range(nwav):
#                amp[:,i] = (amp_ori[:,i*3]+amp_ori[:,i*3+1]+amp_ori[:,i*3+2])/3.0
#                phase[:,i] = (phase_ori[:,i*3]+phase_ori[:,i*3+1]+phase_ori[:,i*3+2])/3.0
...
#return timeobs
    ...
    time = np.transpose(np.tile(np.array([(ttime.mktime((timeobs[i] + datetime.timedelta(days=1)).timetuple()))/(60.0*60.0)
                                          for i in range(len(timeobs))]), [nwav, 1]))
    #print "***"
    #print "time=", time
    #timemjd = np.array([vis_data[i].timemjd/wavelength*wavelength for i in range(len(vis_data))])
    timemjd = np.array([vis_data[i].timemjd for i in range(len(vis_data))])

    print("timemjd=", timemjd)
    print("timemjd.shape=",timemjd.shape)

    timemjd = np.transpose(np.array([timemjd for j in range(nwav)]))

    #print "timemjd=", timemjd[0,:]
    print("timemjd.shape=",timemjd.shape)
...
#        raise TypeError("The time integrations for each visibility are different")
    tint = tint[0]
    tint = tint * np.ones( u.shape )

    # get telescope names for each visibility
    t1 = np.transpose(np.tile( np.array([ vis_data[i].station[0].sta_name for i in range(len(vis_data))]), [nwav,1]))
    t2 = np.transpose(np.tile( np.array([ vis_data[i].station[1].sta_name for i in range(len(vis_data))]), [nwav,1]))
    ...
    el1 = -np.ones(u.shape)
    el2 = -np.ones(u.shape)
    tau1 = -np.ones(u.shape)
    tau2 = -np.ones(u.shape)
    qvis = -np.ones(u.shape)
    uvis = -np.ones(u.shape)
